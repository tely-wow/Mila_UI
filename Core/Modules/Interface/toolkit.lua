...
MilaUI.HandleBlizzardRegions = HandleBlizzardRegions
...
local function GwKill(object)
    if object.UnregisterAllEvents then
        ...
        object:SetParent(MilaUI.HiddenFrame)
    else
...
local function GwCreateBackdrop(frame, template, isBorder, xOffset, yOffset, xShift, yShift)
    ...
    if isBorder then
        ...
        local x = (MilaUI.mult == 1 or (xOffset or 2) == 0) and (xOffset or 2) or
        ((MilaUI.mult < 1 and trunc((xOffset or 2) / MilaUI.mult) or round((xOffset or 2) / MilaUI.mult)) * MilaUI.mult)
        local y = (MilaUI.mult == 1 or (yOffset or 2) == 0) and (yOffset or 2) or
        ((MilaUI.mult < 1 and trunc((yOffset or 2) / MilaUI.mult) or round((yOffset or 2) / MilaUI.mult)) * MilaUI.mult)
...
local function GwHandleDropDownBox(frame, backdropTemplate, hookLayout, dropdownTag, width)
    ...
    if backdropTemplate then
        ...
        frame:GwCreateBackdrop(MilaUI.BackdropTemplates.DefaultWithSmallBorder) -- was StatusBar
    end
    ...
    if text then
        text:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.SMALL)
...
local function GwSkinDropDownMenu(frame, buttonPaddindX, backdropTemplate, textBoxRightOffset)
    ...
    if backdropTemplate then
        ...
        frame:GwCreateBackdrop()
        MilaUI.SkinTextBox(middle, left, right, nil, nil, -5, textBoxRightOffset or -10)
    end
    ...
    button.SetPoint = MilaUI.NoOp
    button:GwStripTextures()

    MilaUI.HandleNextPrevButton(button, "down")
...
MilaUI.HandleNextPrevButton = HandleNextPrevButton

local function GwSetOutside(obj, anchor, xOffset, yOffset, anchor2, noScale)
    if not anchor then anchor = obj:GetParent() end

    if not xOffset then xOffset = MilaUI.BorderSize end
    if not yOffset then yOffset = MilaUI.BorderSize end
    local x = (noScale and xOffset) or MilaUI.Scale(xOffset)
    local y = (noScale and yOffset) or MilaUI.Scale(yOffset)

    if MilaUI.SetPointsRestricted(obj) or obj:GetPoint() then
        obj:ClearAllPoints()
    end
...
local function GwSetFontTemplate(object, font, textSizeType, style, textSizeAddition, skip)
    local settings = MilaUI.DB.profile.Interface
    ...
    if textSizeType == MilaUI.TextSizeType.BIG_HEADER then
        object:SetFont(font, settings.FONTS_BIG_HEADER_SIZE + (textSizeAddition or 0), style or settings.FONTS_OUTLINE)
    elseif textSizeType == MilaUI.TextSizeType.HEADER then
        object:SetFont(font, settings.FONTS_HEADER_SIZE + (textSizeAddition or 0), style or settings.FONTS_OUTLINE)
    elseif textSizeType == MilaUI.TextSizeType.NORMAL then
        object:SetFont(font, settings.FONTS_NORMAL_SIZE + (textSizeAddition or 0), style or settings.FONTS_OUTLINE)
    elseif textSizeType == MilaUI.TextSizeType.SMALL then
        object:SetFont(font, settings.FONTS_SMALL_SIZE + (textSizeAddition or 0), style or settings.FONTS_OUTLINE)
    end

    -- register font for size changes
    MilaUI.texts[object] = true
end
