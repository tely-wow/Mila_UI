...
local function CreateInspectTexture(slot, x, y)
    ...
    if not backdrop.SetBackdrop then
        ...
        backdrop:SetBackdrop(MilaUI.BackdropTemplates.DefaultWithColorableBorder)
    end
...
local function CreateSlotStrings()
    for name, tbl in pairs(InspectItems) do
        ...
        slot.enchantText:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.SMALL, nil, tbl.id >= 11 and -4 or -2)
        ...
        slot.itemlevel:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.SMALL, "THINOUTLINE")
...
local function UpdatePageStrings(inspectItem, slotInfo)
    local width = MilaUI.RoundInt(inspectItem.enchantText:GetWidth())
...
local function TryGearAgain(i, deepScan, inspectItem)
    C_Timer.After(0.05, function()
        local slotInfo = MilaUI.GetGearSlotInfo("player", i, nil, deepScan)
...
do
    local function UpdatePageInfo()
        for name, tbl in pairs(InspectItems) do
            ...
            local slotInfo = MilaUI.GetGearSlotInfo("player", tbl.slotId, nil, true)
            if slotInfo == "tooSoon" then
    ...
    MilaUI.UpdatePageInfo = UpdatePageInfo
end

local function UpdateCharacterInfo(self, event)
    if not MilaUI.DB.profile.Interface.SHOW_CHARACTER_ITEM_INFO then return end
    ...
    if GwCharacterWindow:IsShown() and self.needsUpdate then
        MilaUI.UpdatePageInfo()
        --update itemborder
        for name, _ in pairs(InspectItems) do
            MilaUI.UpdateCharacterPanelItemSlot(_G[name])
        end
...
local function ToggleCharacterItemInfo(setup)
    ...
    if MilaUI.DB.profile.Interface.SHOW_CHARACTER_ITEM_INFO then
        f.needsUpdate = true
...
MilaUI.ToggleCharacterItemInfo = ToggleCharacterItemInfo
