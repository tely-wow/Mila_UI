...
-- Defer accessing the DB until it's needed
local function GetSettings()
    return MilaUI.DB.profile.Interface
end
...
local function LoadBaseFrame()
    ...
    fmGCW.WindowHeader:GwSetFontTemplate(DAMAGE_TEXT_FONT, MilaUI.TextSizeType.BIG_HEADER, nil, 2)
    fmGCW.WindowHeader:SetTextColor(MilaUI.TextColors.LIGHT_HEADER.r,MilaUI.TextColors.LIGHT_HEADER.g,MilaUI.TextColors.LIGHT_HEADER.b)
    ...
    fmGCW:HookScript("OnShow",function(self)
        AddToAnimation("HERO_PANEL_ONSHOW", 0, 1, GetTime(), MilaUI.WINDOW_FADE_DURATION,
        function(p)
            self:SetAlpha(p)
    ...
    local pos = Settings.HERO_POSITION
    local scale = Settings.HERO_POSITION_SCALE
    ...
    fmGCW.sizer:SetScript("OnEnter", function(self)
        ...
        GameTooltip_SetTitle(GameTooltip, "Scale with Right Click")
        GameTooltip:Show()
    end)
    ...
    fmGCW.sizer:SetScript("OnMouseUp", function(self)
        ...
        Settings.HERO_POSITION_SCALE = fmGCW:GetScale()
    ...
    fmGCW.secure:HookScript("OnEvent", function(self, event)
        MilaUI.CombatQueue_Queue("character_update_keybind", click_OnEvent, {self, event})
    end)
...
local function createTabIcon(iconName, tabIndex)
    ...
    f:SetPoint("TOP", GwCharacterWindow, "TOPLEFT", -45, -10 + -((tabIndex - 1) * 45))
...
local function container_OnShow(self)
    ...
    -- No longer setting window icon texture since it's been removed
...
local function LoadCharacter()
    ...
    for _, v in pairs(windowsList) do
        if Settings[v.SettingName] then
            anyThingToLoad = true
        end
    end
    ...
    for _, v in pairs(windowsList) do
        if Settings[v.SettingName] then
            ...
            if MilaUI[v.OnLoad] then
                MilaUI[v.OnLoad](container)
            else
                print("Missing function: " .. v.OnLoad)
            end
...
MilaUI.LoadCharacter = LoadCharacter
...
local function CharacterMenuBlank_OnLoad(self)
    ...
    fontString:GwSetFontTemplate(DAMAGE_TEXT_FONT, MilaUI.TextSizeType.NORMAL)
end
MilaUI.CharacterMenuBlank_OnLoad = CharacterMenuBlank_OnLoad

local function CharacterMenuButton_OnLoad(self, odd)
    if odd == nil then
        odd = MilaUI.nextHeroPanelMenuButtonShadowOdd
    end
    ...
    MilaUI.nextHeroPanelMenuButtonShadowOdd = not MilaUI.nextHeroPanelMenuButtonShadowOdd
end
MilaUI.CharacterMenuButton_OnLoad = CharacterMenuButton_OnLoad
...
MilaUI.CharacterMenuButtonBack_OnLoad = CharacterMenuButtonBack_OnLoad
