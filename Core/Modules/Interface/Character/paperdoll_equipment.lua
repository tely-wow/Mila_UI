local _, MilaUI = ...
MilaUI.char_equipset_SavedItems = {}
local GWGetClassColor = MilaUI.GWGetClassColor
local SetClassIcon = MilaUI.SetClassIcon
local AddToAnimation = MilaUI.AddToAnimation
local IsIn = MilaUI.IsIn
local setItemLevel = MilaUI.setItemLevel
...
local function UpdateAzeriteItem(self)
    if not self.styled then
        ...
        self.RankFrame.Label:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.SMALL, "THINOUTLINE")
    end
end
...
local function updateBagItemButton(button)
    ...
    button.ItemId = id
    button.ItemLink = MilaUI.getContainerItemLinkByNameOrId(name, id) or MilaUI.getInventoryItemLinkByNameAndId(name, id)
...
local function updateItemSlot(self)
    ...
    if MilaUI.char_equipset_SavedItems[slot] == nil then
        MilaUI.char_equipset_SavedItems[slot] = self
...
MilaUI.UpdateCharacterPanelItemSlot = updateItemSlot
...
local function setStatIcon(self, stat)
    ...
    if STATS_ICONS[stat] then
        ...
        if stat == "MASTERY" then
            SetClassIcon(self.icon, MilaUI.myClassID)
...
local function getStatListFrame(self)
    ...
    if not frame.initialized then
        frame.Value:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.NORMAL)
...
local function updateStats(self)
    local average, equipped = MilaUI.GetPlayerItemLevel()
    ...
    local primaryStat = select(6, GetSpecializationInfo(MilaUI.myspec, nil, nil, nil, MilaUI.mysex))
    ...
    for _, category in ipairs(PAPERDOLL_STATCATEGORIES) do
        for _, stat in ipairs(category.stats) do
            ...
            if showStat and stat.roles then
                showStat = tContains(stat.roles, MilaUI.myrole)
            end
    ...
    durabilityFrame:SetScript("OnEnter", MilaUI.DurabilityTooltip)
    durabilityFrame:SetScript("OnEvent", MilaUI.DurabilityOnEvent)
    durabilityFrame:RegisterEvent("UPDATE_INVENTORY_DURABILITY")
    durabilityFrame:RegisterEvent("MERCHANT_SHOW")
    MilaUI.DurabilityOnEvent(durabilityFrame, "ForceUpdate")
end
...
local function updateUnitData(self)
    ...
    local name = select(2, GetSpecializationInfo(MilaUI.myspec, nil, nil, nil, MilaUI.mysex))

    if name ~= nil then
        local data = LEVEL .. " " .. MilaUI.mylevel .. " " .. name .. " " .. MilaUI.myclass
...
local function grabDefaultSlots(slot, anchor, parent, size)
    ...
    MilaUI.RegisterCooldown(_G[slot:GetName()..'Cooldown'])
...
local function ItemLevelTooltip(self)
    local average, equipped, _, averageLocal, equippedLocal, pvpItemLevelLocal = MilaUI.GetPlayerItemLevel()
...
local function LoadPDBagList(fmMenu, parent)
    ...
    local pos = modelPositions[MilaUI.myrace]
    if pos then
    ...
    fmGPDS.header:GwSetFontTemplate(DAMAGE_TEXT_FONT, MilaUI.TextSizeType.NORMAL)
    fmGPDS.header:SetText(STAT_CATEGORY_ATTRIBUTES)
    fmGPDS:SetScript("OnEvent", stats_OnEvent)
    RegisterStatsEvents(fmGPDS)

    fmGDR.characterName:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.HEADER)
    fmGDR.characterData:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.NORMAL)
    fmGDR.itemLevel:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.BIG_HEADER, nil, 6)

    local color = MilaUI.GWGetClassColor(MilaUI.myclass, true)
    SetClassIcon(fmGDR.classIcon, MilaUI.myClassID)
    if color then
...
MilaUI.LoadPDBagList = LoadPDBagList
