local _, MilaUI = ...
local WarningPrompt = MilaUI.WarningPrompt
...
local function updateIngoredSlots(id)
    ...
    for slot, ignored in pairs(ignoredSlots) do
        if (ignored) then
            ...
            MilaUI.char_equipset_SavedItems[slot].ignoreSlotCheck:SetChecked(false)
        else
            C_EquipmentSet.UnignoreSlotForSave(slot)
            MilaUI.char_equipset_SavedItems[slot].ignoreSlotCheck:SetChecked(true)
        end
    end
end
    ...
    MenuUtil.CreateContextMenu(self, function(ownerRegion, rootDescription)
        ...
        do
            for i = 1, GetNumSpecializations() do
                ...
                check:AddInitializer(MilaUI.BlizzardDropdownCheckButtonInitializer)
            end
        end
    end)
end
...
local function EquipmentSet_InitButton(button, elementData)
    if not button.isSkinned then
        ...
        button.name:GwSetFontTemplate(UNIT_NAME_FONT, MilaUI.TextSizeType.NORMAL)
        ...
        MilaUI.AddListItemChildHoverTexture(button)
...
local function LoadPDEquipset(fmMenu, parent)
    ...
    hooksecurefunc(GwGearManagerPopupFrame, "OnShow", function(frame)
        if not frame.isSkinned then
            MilaUI.HandleIconSelectionFrame(frame)
        end
    end)
    ...
    MilaUI.HandleTrimScrollBar(fmGPDO.ScrollBar)
    MilaUI.HandleScrollControls(fmGPDO)
...
MilaUI.LoadPDEquipset = LoadPDEquipset
