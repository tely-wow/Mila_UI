...
MilaUI.SkinTextBox = SkinTextBox
...
MilaUI.MutateInaccessableObject = MutateInaccessableObject

local NavBarCheck = {
    EncounterJournal = function()
        return MilaUI.settings.ENCOUNTER_JOURNAL_SKIN_ENABLED
    end,
    WorldMapFrame = function()
        return MilaUI.settings.WORLDMAP_SKIN_ENABLED
    end,
}
...
MilaUI.HandlePortraitFrame = HandlePortraitFrame
...
do
    ...
    local function iconBorderColorAtlas(border, atlas)
        ...
        local color = MilaUI.GetBagItemQualityColor(iconColors[atlas])
    ...
    local function HandleIconBorder(border, backdrop, customFunc)
        ...
        local r, g, b, a = border:GetVertexColor()
        local quality = iconColors[border:GetAtlas()]
        local atlas = quality and MilaUI.GetBagItemQualityColor(quality)
        if customFunc then
    ...
    MilaUI.HandleIconBorder = HandleIconBorder
end
local function GetScale()
    MilaUI.scale = UIParent:GetScale()
end
MilaUI.GetScale = GetScale
local function GetMult()
    MilaUI.GetScale()
    return (1 / MilaUI.scale) - ((1 - (768 / MilaUI.screenHeight)) / MilaUI.scale)
end
MilaUI.GetMult = GetMult
local function Scale(x)
    -- Add nil check to prevent errors
    if x == nil then
        return 0
    end
    
    local m = MilaUI.GetMult()
    if m == 1 or x == 0 then
        return x
    else
...
local function ReskinScrollBarArrow(frame, direction)
    MilaUI.HandleNextPrevButton(frame, direction)
...
MilaUI.HandleScrollControls = HandleScrollControls
...
MilaUI.HandleTrimScrollBar = HandleTrimScrollBar
...
local function HandleItemButton(b, setInside)
    ...
    b:GwStripTextures()
    b:GwCreateBackdrop(MilaUI.BackdropTemplates.DefaultWithSmallBorder, true)
    b:GwStyleButton()
...
MilaUI.HandleItemButton = HandleItemButton
...
do
    ...
    local function HandleIconSelectionFrame(frame)
        ...
        frame:GwStripTextures()
        frame:GwCreateBackdrop(MilaUI.BackdropTemplates.DefaultWithSmallBorder, true)
        ...
        if borderBox then
            ...
            if button then
                ...
                MilaUI.HandleItemButton(button, true)
            end
        end
        ...
        if editBox then
            MilaUI.SkinTextBox(editBox.IconSelectorPopupNameMiddle, editBox.IconSelectorPopupNameLeft, editBox.IconSelectorPopupNameRight, nil, nil, 5, 5)
        end

        MilaUI.HandleTrimScrollBar(frame.IconSelector.ScrollBar)
        MilaUI.HandleScrollControls(frame.IconSelector)
    ...
    MilaUI.HandleIconSelectionFrame = HandleIconSelectionFrame
end
...
local function CreateFrameHeaderWithBody(frame, titleText, icon, detailBackgrounds, detailBackgroundsXOffset, addLeftSidePanel, addFrameOpenAnimation)
    ...
    if titleText then
        ...
        titleText:GwSetFontTemplate(DAMAGE_TEXT_FONT, MilaUI.TextSizeType.BIG_HEADER, nil, 2)
        titleText:SetTextColor(MilaUI.TextColors.LIGHT_HEADER.r,MilaUI.TextColors.LIGHT_HEADER.g,MilaUI.TextColors.LIGHT_HEADER.b)
    end
    ...
    if addFrameOpenAnimation then
        ...
        MilaUI.AddToAnimation((frame.GetName and frame:GetName() or tostring(frame)) .. "_PANEL_ONSHOW", 0, 1, GetTime(), MilaUI.WINDOW_FADE_DURATION,
            function(p)
                frame:SetAlpha(p)
                bgMask:SetPoint("BOTTOMRIGHT", frame.tex, "BOTTOMLEFT", MilaUI.lerp(-64, frame.tex:GetWidth(), p), 0)
            end, 1, function()
...
MilaUI.CreateFrameHeaderWithBody = CreateFrameHeaderWithBody
...
MilaUI.AddDetailsBackground = AddDetailsBackground
    ...
    for i, header in next, { frame.HeaderContainer:GetChildren() } do
        if not header.IsSkinned then
            ...
            if not header.backdrop then
                header:GwCreateBackdrop(MilaUI.BackdropTemplates.DefaultWithColorableBorder, true)
...
MilaUI.HandleSrollBoxHeaders = HandleHeaders
...
local function HandleScrollFrameHeaderButton(button, isLastButton)
    if not button.IsSkinned then
        ...
        if not button.backdrop then
            button:GwCreateBackdrop(MilaUI.BackdropTemplates.DefaultWithColorableBorder, true)
...
MilaUI.HandleScrollFrameHeaderButton = HandleScrollFrameHeaderButton
...
MilaUI.AddMouseMotionPropagationToChildFrames = AddMouseMotionPropagationToChildFrames
...
local function AddListItemChildHoverTexture(child)
    ...
    if child.HighlightTexture then
        ...
        child:HookScript("OnEnter", function()
            MilaUI.TriggerButtonHoverAnimation(child, child.HighlightTexture)
        end)
    elseif child.Highlight then
        ...
        child:HookScript("OnEnter", function()
            MilaUI.TriggerButtonHoverAnimation(child, child.Highlight)
        end)
        ...
        child:HookScript("OnEnter", function()
            ...
            MilaUI.TriggerButtonHoverAnimation(child, child.gwHoverTexture)
        end)
...
MilaUI.AddListItemChildHoverTexture = AddListItemChildHoverTexture
...
MilaUI.HandleItemListScrollBoxHover = HandleItemListScrollBoxHover
...
MilaUI.SkinSideTabButton = SkinSideTabButton
...
MilaUI.LockBlackButtonColor = LockBlackButtonColor
...
MilaUI.LockWhiteButtonColor = LockWhiteButtonColor
