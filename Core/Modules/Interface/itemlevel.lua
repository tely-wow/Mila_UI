local _, MilaUI = ...
local RoundDec = MilaUI.RoundDec
...
local function PopulateUnitIlvlsCache(unitGUID, itemLevel, tooltip)
    if itemLevel then
        if MilaUI.unitIlvlsCache[unitGUID] then
            MilaUI.unitIlvlsCache[unitGUID].time = GetTime()
            MilaUI.unitIlvlsCache[unitGUID].itemLevel = itemLevel
        end
...
MilaUI.PopulateUnitIlvlsCache = PopulateUnitIlvlsCache
...
do
    ...
    MilaUI.GetGearSlotInfo = GetGearSlotInfo
end
...
MilaUI.CalculateAverageItemLevel = CalculateAverageItemLevel
...
local function GetPlayerItemLevel()
    ...
    averageLocal = MilaUI.GetLocalizedNumber(average)
    equippedLocal = MilaUI.GetLocalizedNumber(equipped)
    pvpItemLevelLocal = MilaUI.GetLocalizedNumber(pvpItemLevel)

    return average, equipped, pvpItemLevel, averageLocal, equippedLocal, pvpItemLevelLocal
end
MilaUI.GetPlayerItemLevel = GetPlayerItemLevel

do
    local iLevelDB, tryAgain, slotInfo = {}, {}, nil
    local function GetUnitItemLevel(unit)
        if UnitIsUnit(unit, "player") then
            local _, equipped = MilaUI.GetPlayerItemLevel()
			return equipped
        end
        ...
        for i = 1, 17 do
            if i ~= 4 then
                ...
                slotInfo = MilaUI.GetGearSlotInfo(unit, i)
    ...
    MilaUI.GetUnitItemLevel = GetUnitItemLevel
end