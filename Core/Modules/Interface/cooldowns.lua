...
do
    ...
    MilaUI.GetTimeInfo = GetTimeInfo
end
...
local function Cooldown_OnUpdate(self, elapsed)
    ...
    if not Cooldown_IsEnabled(self) then
        ...
        elseif self.endTime then
            local text, nextUpdate = MilaUI.GetTimeInfo(self.endTime - now, true)
            if not forced then
...
MilaUI.ToggleBlizzardCooldownText = ToggleBlizzardCooldownText
...
MilaUI.RegisterCooldown = RegisterCooldown
