...
local IsFrameModified = MilaUI.IsFrameModified
local IsIncompatibleAddonLoadedOrOverride = MilaUI.IsIncompatibleAddonLoadedOrOverride
local Debug = MilaUI.Debug
local AFP = MilaUI.AddProfiling
local Settings = MilaUI.profile.Interface
...
MilaUI.VERSION_STRING = "GW2_UI 7.15.0"
...
local function disableMABags()
    local bags = Settings.BAGS_ENABLED and not IsIncompatibleAddonLoadedOrOverride("Inventory", true)
...
local function evPlayerEnteringWorld()
    ...
    if dbMigrated then
        C_Timer.After(3, function() GW.WarningPrompt(
            "DB was converted Reload is needed /reload",
...
local function evPlayerLogin(self)
    ...
    if (forcedMABags) then
        GW.Notice("MoveAnything bag handling disabled.")
    end
